...
## Summary
When the Rujira Deployer Multisig updates interest rates via `SudoMsg::SetInterest`, the `sudo()` function fails to call `distribute_interest()` before applying the new rate configuration. This causes accrued interest for the period since the last update to be calculated using the NEW rate instead of the OLD rate when the next transaction occurs, resulting in systematic over- or under-charging of interest to borrowers and incorrect returns to depositors.
...
The `calculate_interest()` function uses the passed interest configuration to determine the rate, and calculates interest for the time period since `last_updated`. [4](#0-3) 

The interest rate is calculated based on utilization using the Interest struct's rate() method. [5](#0-4) 
...
2. **Time T0 + 180 days**: Admin calls `sudo(SetInterest)` to change rate to 5% APR
   - Config updated to 5% APR at line 215
   - State NOT updated, `last_updated` still = T0
   - No `distribute_interest()` called
3. **Time T0 + 181 days**: Next user transaction (deposit/withdraw/borrow/repay)
   - `execute()` calls `distribute_interest()` at line 51
   - `distribute_interest()` calls `calculate_interest(&config.interest, ...)` with NEW config
...
1. **Direct Economic Loss**: Depositors lose earned interest when rates decrease (most common scenario), borrowers are overcharged when rates increase
...
4. **Protocol Revenue Loss**: If rates decrease significantly, the protocol loses substantial fee revenue on the miscalculated interest

Using realistic parameters:
- Vault with $10M TVL
- 80% utilization ($8M borrowed)
- Rate change from 15% to 3% APR (responding to market conditions)
- 90 days until next transaction (low activity period)
- Lost interest: $8M × (0.15 - 0.03) × 90/365 = **$236,712**

## Likelihood Explanation
**HIGH LIKELIHOOD** - This will occur every time interest rates are updated:

1. **Frequent Operation**: Interest rate updates are a normal governance action responding to market conditions, vault performance, and protocol economics
...
4. **Worse During Low Activity**: The impact compounds during low vault activity when time between transactions is longer

## Recommendation
Modify the `sudo()` function to call `distribute_interest()` before updating the interest configuration:

```rust
SudoMsg::SetInterest(interest) => {
    interest.validate()?;
    
    // Load state and distribute interest at old rate before updating
    let mut state = State::load(deps.storage)?;
    let fees = state.distribute_interest(&_env, &config)?;
    state.save(deps.storage)?;
    
    // Now update to new interest rate
    config.interest = interest;
    config.save(deps.storage)?;
    
    // Optionally mint protocol fees if any
    let mut response = Response::default();
    if fees.gt(&Uint128::zero()) {
        let rcpt = TokenFactory::new(&_env, format!("ghost-vault/{}", config.denom).as_str());
...
```rust
#[test]
fn test_interest_rate_change_miscalculation() {
    ...
    app.init_modules(|router, _, storage| {
        ...
        router
            .bank
            .init_balance(storage, &borrower, coins(10_000_000, "btc"))
            .unwrap();
    });
    ...
    // Wait 180 days at 20% APR
    app.update_block(|x| x.time = x.time.plus_days(180));

    // Admin changes rate to 5% without distributing interest
    app.wasm_sudo(
        contract.clone(),
        &SudoMsg::SetInterest(Interest {
            target_utilization: Decimal::from_ratio(8u128, 10u128),
    ...
    // Wait 1 more day at 5% APR
    app.update_block(|x| x.time = x.time.plus_days(1));

    // Query status to trigger interest distribution
    let status: StatusResponse = app
        .wrap()
        .query_wasm_smart(contract.clone(), &QueryMsg::Status {})
        .unwrap();

    // Expected: (1,000,000 * 0.20 * 180/365) + (1,000,000 * 0.05 * 1/365) = 98,630 + 137 = 98,767
    // Actual: 1,000,000 * 0.05 * 181/365 = 24,794
    // Loss: ~73,973 tokens

    // Debt should be ~1,098,767 but will be ~1,024,794
    println!("Actual debt pool size: {}", status.debt_pool.size);
    println!("Expected debt pool size: ~1,098,767");
    println!("Loss to depositors: ~73,973 tokens");

    // The bug causes significant undercounting of interest
    assert!(
        status.debt_pool.size < Uint128::from(1_030_000u128),
        "Interest miscalculation: debt too low"
    );
}
```

This test demonstrates that when interest rates are changed without distributing accrued interest first, the entire period gets calculated at the new rate, causing substantial financial loss to depositors.

### Citations

**File:** contracts/rujira-ghost-vault/src/contract.rs (L42-52)
```rust
pub fn execute(
    deps: DepsMut,
    env: Env,
...
**File:** packages/rujira-rs/src/interfaces/ghost/vault/interest.rs (L49-67)
```rust
